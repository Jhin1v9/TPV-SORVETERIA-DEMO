async (page) => {
  const sql = "
-- Essential functions only (no reset_demo_data)
...
begin
  ...
  -- Primeira passada: validaÃƒÂ§ÃƒÂ£o e cÃƒÂ¡lculo de subtotal
  ...
  loop
    ...
    if is_legacy then
      -- Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Formato legado Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
      category_sku := item_record->'categoria'->>'id';
      ...
      -- Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Formato novo (produto) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
-- Gera um cÃƒÂ³digo de 5 dÃƒÂ­gitos para vincular pedido kiosk ao perfil PWA
...
declare
  ...
  code_exists boolean;
begin
  -- Invalida cÃƒÂ³digos antigos do mesmo customer
  update public.kiosk_codes
  set used_at = now()
  where customer_id = customer_id_input and used_at is null;

  -- Gera cÃƒÂ³digo ÃƒÂºnico de 5 dÃƒÂ­gitos
  loop
...
-- Valida um cÃƒÂ³digo de kiosk e retorna o customer_id
...
begin
  ...
  if matched_id is null then
    raise exception 'CÃƒÂ³digo invÃƒÂ¡lido ou expirado';
  end if;
