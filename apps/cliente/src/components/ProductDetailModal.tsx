...
function inferirLimitesDoTamanho(tamanhoNome: string): {
  sabores: number | null;
  toppings: number | null;
  frutas: number | null;
} {
  const saboresMatch = tamanhoNome.match(/(\d+)\s*(?:bola|sabor|boles|sabors|sabores|scoops|flavor)/i);
  const toppingsMatch = tamanhoNome.match(/(\d+)\s*(?:toppings?|topping)/i);
  const frutasMatch = tamanhoNome.match(/(\d+)\s*(?:frutas?|fruta)/i);
  return {
    sabores: saboresMatch ? parseInt(saboresMatch[1], 10) : null,
    toppings: toppingsMatch ? parseInt(toppingsMatch[1], 10) : null,
    frutas: frutasMatch ? parseInt(frutasMatch[1], 10) : null,
  };
}
...
export default function ProductDetailModal({ produto, onClose }: ProductDetailModalProps) {
  ...
  // ─── Limites dinâmicos baseados no tamanho selecionado ───
  const tamanhoSelecionado = selecoes['tamanhos']?.[0];
  const limitesInferidos = tamanhoSelecionado
    ? inferirLimitesDoTamanho(tamanhoSelecionado.nome[locale] || tamanhoSelecionado.nome.es)
    : { sabores: null, toppings: null, frutas: null };

  const maxSabores = limitesInferidos.sabores ?? (isPersonalizavel ? produto.limites?.maxSabores ?? 3 : 0);
  const maxToppings = limitesInferidos.toppings ?? (isPersonalizavel ? produto.limites?.maxToppings ?? 4 : 0);
  const maxFrutas = limitesInferidos.frutas ?? (isPersonalizavel ? produto.limites?.maxFrutas ?? 3 : 0);
  const maxExtras = 5;

  // ─── Toggle opções ───
  const toggleOpcao = useCallback((tipo: string, opcao: OpcaoPersonalizacao, max: number) => {
    setSelecoes((prev) => {
  ...
  const selecionarTamanho = useCallback((opcao: OpcaoPersonalizacao) => {
    setSelecoes((prev) => {
  ...
  const handleScroll = useCallback((e: React.UIEvent<HTMLDivElement>) => {
    setHeaderCompact(e.currentTarget.scrollTop > 180);
  }, []);
  ...
  const handleAdd = useCallback(() => {
    ...
    handleClose();
  }, [addToCarrinho, calcularTotal, handleClose, produto, selecoes]);
  ...
  return (
    ...
    >
      <div
        className={`bg-white w-full max-w-lg sm:rounded-3xl rounded-t-3xl max-h-[92vh] overflow-hidden flex flex-col transition-transform duration-200 ease-out ${
      ...
      >
        ...
        <div
          className={`absolute top-0 left-0 right-0 z-20 bg-white/95 backdrop-blur-md border-b border-gray-100 px-5 py-3 flex items-center justify-between transition-all duration-200 ${
            headerCompact ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 pointer-events-none'
          }`}
        >
