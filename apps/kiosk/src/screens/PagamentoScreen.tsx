  ...
  return (
    <div className="h-full flex flex-col bg-[#FAFAFA]">
      <div className="flex items-center justify-between px-8 py-7">
        ...
        <h1 className="font-display text-3xl font-bold text-gray-800">{t('payment', locale)}</h1>

        <div className="font-mono font-bold text-2xl text-[#FF6B9D]">EUR {summary.total.toFixed(2)}</div>
      </div>
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'tarjeta' && (
            <motion.div key="tarjeta" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="flex flex-col items-center justify-center h-full">
              {!busy ? (
                <>
                  <div className="w-96 h-56 bg-gradient-to-br from-[#2D3436] to-[#636E72] rounded-2xl shadow-xl p-6 relative overflow-hidden mb-8">
                    ...
                    <div className="font-mono text-white text-xl tracking-widest mb-4">**** **** **** 4242</div>
                    <div className="flex justify-between">
                      <div>
                        <p className="text-white/40 text-xs uppercase">Titular</p>
                        <p className="text-white text-sm">CLIENTE SABADELL</p>
                      </div>
                      <div>
                        <p className="text-white/40 text-xs uppercase">Expira</p>
                        <p className="text-white text-sm">12/28</p>
                      </div>
                    </div>
                  </div>

                  <motion.button onClick={handleTarjeta} className="h-20 px-12 rounded-2xl bg-[#2D3436] text-white font-display font-bold text-xl shadow-lg flex items-center gap-3" whileTap={{ scale: 0.98 }}>
                    <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          {activeTab === 'efectivo' && (
            <motion.div key="efectivo" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="flex flex-col items-center justify-center h-full">
              <div className="bg-white rounded-3xl p-8 shadow-lg flex flex-col items-center">
                <QRCodeSVG value={qrData} size={240} level="M" bgColor="#FFFFFF" fgColor="#2D3436" includeMargin={false} />
                <p className="font-mono font-bold text-xl text-gray-800 mt-4">EUR {summary.total.toFixed(2)}</p>
              </div>
              ...
              <div className="mt-6 flex items-center gap-3 text-gray-500 bg-gray-100 rounded-xl px-6 py-3">
                ...
                <p className="text-base">{t('payAtCounter', locale)}</p>
              </div>

              <motion.button onClick={handleCash} className="mt-8 h-16 px-10 rounded-2xl bg-[#4CAF50] text-white font-display font-bold text-xl shadow-lg disabled:opacity-60" whileTap={{ scale: 0.98 }} disabled={busy}>
                Confirmar pedido
              </motion.button>
            </motion.div>
          )}
          ...
          {activeTab === 'bizum' && (
            <motion.div key="bizum" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="flex flex-col items-center justify-center h-full">
              ...
              <h3 className="font-display text-2xl font-bold text-gray-800 mb-2">Bizum</h3>
              <p className="text-gray-400 text-base mb-6">O telefone fica gravado no pedido real da sessao</p>
              ...
              <input
                ...
                className="w-80 h-14 text-center text-2xl font-mono border-2 border-gray-200 rounded-2xl focus:border-[#4CAF50] outline-none mb-6"
                maxLength={9}
              />
              ...
              <motion.button
                ...
                className="h-16 px-10 rounded-2xl font-display font-bold text-xl shadow-lg transition-all disabled:opacity-50 bg-[#4CAF50] text-white"
